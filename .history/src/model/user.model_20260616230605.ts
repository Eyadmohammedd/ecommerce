    ...
    useFactory: (securityService: SecurityService) => {
      ...
      UserSchema.pre('save', async function (this: UserDocument) {
        ...
        if (this.isModified('password') && this.password) {
          this.password = await this.securityService.generateHash({ plaintext: this.password });
        }

        if (this.phone && this.isModified('phone')) {
          this.phone = this.securityService.generateEncryption(this.phone);
        }
      ...
      UserSchema.pre(
        ...
        async function (this: any) {
          ...
          if (set.phone) {
            set.phone = generateEncryption(set.phone);
            update.$set = set;
          }
